...
\para 设$U$是$\rr^n$中的开集，如果$f:M\to U$是一个连续函数，则在$M$的每一点$x$，我们连续地找一个$\epsilon(x)>0$使得以$f(x)$为球心的，$\epsilon(x)$为半径的开球完全处于$U$内，那么按上面的引理，我们可以找到$M$上的一个光滑函数$g$使得$\|g(x)-f(x)\|<\epsilon(x)$对$x\in M$恒成立，则我们定义$H(x,t)=tf(x)+(1-t)g(x)$，由于
\[
