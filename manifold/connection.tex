...
\begin{compactenum}
	\item 从一点向另一点可以引一条直线。
	\item 任意线段能无限延伸成一条直线。
	\item 给定任意线段，可以以其一个端点作为圆心，该线段作为半径作一个圆。
	\item 所有直角都相等。
	\item 给定一条直线，通过此直线外的任何一点，有且只有一条直线与之平行。
\end{compactenum}
任何几何都可以看成欧氏几何的推广，比如我们下面讨论的流形上的几何。但是，正如欧氏几何的几大公理中所体现的，为了给出所谓的“几何”，我们首先得回答几个问题：什么是点？什么是直线？这两个是几何的基本对象。当然，对应欧氏几何最后一个公理（也被称为平行公理），我们还得回答，什么是平行？这也可以理解为几何的一个基本关系。其余的基本关系比这要平凡些，比如点是否在直线上等。

给定一个流形，“点”是不言自明的，但是对“直线”与“平行”我们都没有相关的概念，对于直线，我们只能说它是一条“曲线”。所以为了给出流形上的几何，我们需要给出“直线”与“平行”的概念。
\end{para}
...
\begin{compactenum}
	...
	\item 任意两点的切矢量都平行的曲线。
\end{compactenum}
这两种刻画都非常深刻，前者对应于所谓的“测地线”，也体现为变分原理，但是为了得到这种刻画，我们需要在流形上引入可以导出“长度”的结构，比如Riemann度量，后者对应于所谓的“自平行曲线”，但我们也需要引入其他结构来告诉我们，两个切矢量什么时候是平行的。

在欧氏空间中，上面两种刻画几乎是等价的。在Riemann流形（或更一般的伪Riemann流形）上，我们将会看到，利用度量，我们可以定义出什么是平行。所以，在直线的两种刻画中，后者相对更为基础，同时也更为直观些。
\end{para}

所以，在某种程度上，为了回答什么是直线，我们只需回答什么是平行即可。

\begin{para}[平行移动]
现在让我们回到欧氏几何，或更一般的矢量空间$V$中。$V$中的两个矢量是否平行被直接定义为它们是否线性相关。

现在，假设我们有一条曲线$\sigma:\rr\to \rr^n$，那么它每一点的切矢量有
\[
	\dot\sigma(t)\in T_{\sigma(t)}\rr^n,
\]
每一点的切空间$T_{\sigma(t)}\rr^n$实际上都是不同的矢量空间，我们可以把$T_{\sigma(t)}\rr^n$看成$\sigma(t)$处放了一个$\rr^n$. 在这里，因为不同切矢量所处的空间不同，我们不能直接利用$\rr^n$中两个矢量的平行的定义。

但是，在非常久远之前，我们将所有的$T_{\sigma(t)}\rr^n$都等同于$\rr^n$，这个过程在这里可以理解为将$T_{\sigma(t)}\rr^n$中的矢量“移到”同一个矢量空间$T_{0}\rr^n$中。此时，利用$T_{0}\rr^n$上面平行的定义，我们也就得到了不同切空间中两个切矢量的平行与否。

在一般的流形$M$上，假设我们有一条曲线$\sigma:\rr\to M$，像之前一样，每一点的切矢量都在不同的切空间之中。所以为了定义平行，我们需要定义什么叫做“平行移动”。
\end{para}
