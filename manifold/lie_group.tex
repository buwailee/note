...
\]
那么直接计算特征标就可以得到，对于紧Lie群的不可约幺正表示，我们有$(\chi_\pi, \chi_\pi^*)=1$，对于两个不可约的不等价表示，我们有$(\chi_\pi, \chi_\rho^*)=1$，其中上标$*$表示复共轭。
...
设$f:G\to G$是一个微分同胚，我们有$f_*[X,Y]=[f_*X,f_*Y]$，若$f=l_a$，那么我们立刻就得到了左不变矢量场的对易子也是左不变的。因此对于Lie代数来说，他容许一个二元线性运算$[\bullet,\bullet]:\lag\times \lag\to \lag$，所以Lie代数确实是一个代数。
...
\[
	\begin{split}
	(X^{k+1}f)(x e^{tX})&=(X(X^{k}f))(x e^{tX})=\left\{\frac{\dd}{\dd s}(X^kf)(x e^{(s+t)X})\right\}_{s=0}\\
	&=\frac{\dd}{\dd t}(X^kf)(x e^{tX})=\frac{\dd^{k+1}}{\dd t^{k+1}}g(t).
	\end{split}
\]
那么使用Taylor公式
\[
...
\theo 对于$A\in\mathfrak{gl}(n,\rr)$，指数映射有如下级数展开
\[
...
\para $G$是一个Lie群，他的切丛记做$TG$，形式$\omega_G:v\mapsto (l_{g^{-1}})_*v$被称为Maurer-Cartan形式。
...
所以一般线性群上面的Maurer-Cartan形式即为$\omega_G(v)=l_{g^{-1}}(v)=g^{-1}v$，其中$g$和$v$都是矩阵，矩阵乘矩阵还是矩阵，所以Lie代数也是矩阵的形式。设$\dd g=(\dd x_{ij})$，那么$v$就可以写成$\dd g(v)$，因为$\dd x_{ij}(v)=v_{ij}$，则$\omega_G=g^{-1}\dd g$.
