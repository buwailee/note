...
但我们现在有一个严峻的问题，$X$的子概形的子概形是否还是$X$的子概形？一个定义良好的子对象，至少应该满足是满足这点。答案是令人愉悦的，这直接来自于下面的几个命题。
...
\begin{coro}
$X$的（闭）子概形的（闭）子概形可以典范等同于一个$X$的（闭）子概形。
\end{coro}

\begin{proof}
设$Y$是$X$的子概形，而$Z$是$Y$的子概形。开子概形的开子概形就是一个开子概形，所以开子概形的子概形也是一个子概形。下面我们假设$Y$是$X$的闭子概形。

现在找一个$Y$在$X$中的仿射开覆盖$\{U_\alpha\}$，对每一个$\alpha$，$U_\alpha\cap Y$都是$U_\alpha$的一个闭子概形。由于$Z$是$Y$的一个子概形，所以可以在$\{U_\alpha\cap Y\}$中选取那些使得$Z$为$U_\alpha\cap Y$中闭集的开集，它们构成了$Z$在$Y$中的一个开覆盖。此时，$U_\alpha\cap Y\cap Z$就是$U_\alpha\cap Y$的一个闭子概形，而$U_\alpha\cap Y$是$U_\alpha$的闭子概形。因此，我们只要说明仿射概形$\spec R$的闭子概形的闭子概形可以典范等同于$\spec R$的一个闭子概形即可。

设$V(\mathfrak{a})=\spec(R/\mathfrak{a})$是$\spec R$的一个闭子概形，而$V(\overline{\mathfrak{b}})=\spec((R/\mathfrak{a})/\overline{\mathfrak{b}})$是$V(\mathfrak{a})$的一个闭子概形。取$\overline{\mathfrak{b}}$在$R$中的原像$\mathfrak b$，典范等同$(R/\mathfrak{a})/\overline{\mathfrak b}=R/\mathfrak{b}$告诉我们$V(\overline{\mathfrak{b}})$可以典范等同于$\spec R$中的闭子概形$V(\mathfrak{b})$.
\end{proof}
...
\]
再利用$\psi$是一个拓扑空间的含入映射（这意味着$Y$中的开集都可以写成某个$X$中开集在含入映射下的原像），$\psi_*(\theta_1)=\psi_*(\theta_2)$给出$\theta_1=\theta_2$. 所以闭子概形的典范含入态射是一个单态。
...
\]
则称$f$被概形$Y$的含入态射所遮盖。由于典范含入是单态，所以对于给定的$f$和子概形$Y$，分解总是唯一的。
\end{para}

\begin{pro}
态射$f:Z\to X$被含入$Y\hookrightarrow X$所遮盖，当且仅当，$f(Z)\subset Y$且对任意的$z\in Z$，$f$诱导的同态$(\oo_{X})_{f(z)}\to (\oo_{Z})_z$总可以分解为$(\oo_{X})_{f(z)}\to (\oo_Y)_{f(z)}\to (\oo_{Z})_z$.
\end{pro}
...
\]
令$f=(\psi,\theta)$，并设$\mathscr{J}$是$\theta^\#:\psi^*\oo_X\to \oo_Z$的核，他是$\psi^*\oo_X$的一个理想层，局部地，写作$\theta^\#_z:(\oo_X)_{f(z)}\to (\oo_Z)_z$. 由假设，它可以分解为
\[
...
\]
即$\psi^*(\mathscr I)\subset \mathscr J$. 进而，利用余核的泛性质，$\theta^\#$可以分解为
\[
	\psi^*(\oo_X)\to \psi^*(\oo_X)/\psi^*(\mathscr I)=\psi^*(\oo_X/\mathscr I)\xrightarrow{\omega} \oo_Z
\]
...
\begin{coro}
如果$Y$和$Z$都是$X$的子概形，则含入$Z\hookrightarrow X$被含入$Y\hookrightarrow X$所遮盖当且仅当$Z$是$Y$的子概形。
\end{coro}

\begin{proof}
如果$Z$是$Y$的子概形，则任取$z\in Z$，上一个命题中的分解都是成立的，所以$Z\hookrightarrow X$被含入$Y\hookrightarrow X$所遮盖。

反过来，如果含入$i:Z\hookrightarrow X$被含入$j:Y\hookrightarrow X$所遮盖，因为存在分解$i:Z\to  Y\xrightarrow{j} X$，由于$Y$和$Z$都是子概形，所以商同态$i_z:(\oo_X)_z\to (\oo_X)_z/(\mathscr I_Z)_z$有分解
\[
	i_z:(\oo_X)_z\xrightarrow{j_z} (\oo_X)_z/(\mathscr I_Y)_z\to (\oo_X)_z/(\mathscr I_Z)_z
\]
其中$j_z$也是商同态，而$\mathscr I_Y$和$\mathscr I_Z$分别是某两个开集上的拟凝聚理想层。由余核$j_z$的泛性质，分解存在必然唯一，所以$(\oo_X)_z/(\mathscr I_Y)_z\to (\oo_X)_z/(\mathscr I_Z)_z$就是一个商同态，写作
\[
(\oo_X)_z/(\mathscr I_Y)_z\to \left((\oo_X)_z/(\mathscr I_Y)_z\right)/\left((\mathscr I_Z)_z/(\mathscr I_Y)_z\right)=(\oo_X)_z/(\mathscr I_Z)_z.
\]
所以分解中的$Z\to Y$是一个典范含入，即$Z$是$Y$的一个子概形。
\end{proof}

所以我们在$X$所有子概形中引入如下偏序：$Z\leq Y$如果含入$Z\hookrightarrow X$被含入$Y\hookrightarrow X$所遮盖，或而言之，$Z$是$Y$的子概形。

\begin{para}[浸入]
设$X$和$Y$是两个概形，而$f:Y\to X$是一个态射，如果$f$被一个含入$Z\hookrightarrow X$所遮盖，即存在分解$f:Y\xrightarrow{g} Z\hookrightarrow X$，其中$g$是一个同构，则称$f$是一个浸入。如果$Z$是开（闭）子概形，则称$f$为开（闭）浸入。

如果$f:Y\to X$是一个浸入，则子概形$i:Z\hookrightarrow X$和同构$g$都是唯一确定的。实际上，如果有另一个子概形$i':Z'\hookrightarrow X$和同构$g'$，则我们有$ig=i'g'$，那么$i=i'g'g^{-1}$，故$Z\leq Z'$. 反过来也给出$Z'\leq Z$，所以$Z=Z'$且$i=i'$. 再利用典范含入是单的，则$g=g'$.
\end{para}

类似于子概形的典范含入，浸入也是概形范畴的单态。
...
\begin{pro}[子概形的原像]
设$f:X\to Y$是一个概形间态射，$i:Y'\hookrightarrow Y$的（闭、开）子概形，则投影$p_X:X\times_Y Y'\to X$是一个（闭、开）浸入，它唯一确定了$X$的一个（闭、开）子概形$X'$，$X'$作为拓扑空间同胚于$f^{-1}(Y')$.
\end{pro}
