...
\begin{proof}
	首先，任取$x\in Y'$，则作为连续映射，我们有$x=i'(x)=i(g(x))=g(x)$，所以$g$作为连续映射只能为恒等映射。所以，只需将$X$换成包含$Y=Y'$为闭集的最大开集，则我们可以进一步假设$Y$和$Y'$都是$X$的闭子概形，对应的拟凝聚理想层为$\mathscr I$和$\mathscr I'$.

	任取$x\in |Y|=|Y'|$，我们有分解$(i')^\#_x=g^\#_x i^\#_x$. 由闭子概形的构造，$i^\#_x:(\oo_X)_x\to (\oo_X)_x/\mathscr I_x$和$(i')^\#_x:(\oo_X)_x\to (\oo_X)_x/\mathscr I'_x$都是商同态。任取$s_x\in \mathscr I_x$，分解告诉我们$(i')^\#_x(s_x)=0$当且仅当$i^\#_x(s_x)=0$，所以$\mathscr I'_x\subset \mathscr I_x$. 此外，在$|Y|$外，有$\mathscr I'_x=(\oo_X)_x=\mathscr I_x$，因此，$\mathscr I=\mathscr I'$. 而这意味着，$Y'$和$Y$就是同一个闭子概形，此即所证。
\end{proof}
...
\begin{pro}
	设$f:X\to Y$是概形态射，如果$f_*\oo_X$是拟凝聚的$\oo_Y$-模层，则$f$的闭像存在。
\end{pro}
