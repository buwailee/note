...
\[
	f_V=ihg_U|_{\psi^{-1}(V)},
\]
其中$i$是一个子概形的典范含入，从浸入分解的唯一性，$i=j_V$，再利用一次浸入分解的唯一性，$j_V$或子概形$Z_V$由$j_U|_{Z'_V}$唯一确定。类似地，$g_V$由$g_U|_{\psi^{-1}(V)}$唯一确定，这些就是黏合条件。此时，将$Z_U$以及$g_U$黏合起来，我们有闭子概形$j:Z\hookrightarrow X$以及同构$g:Y\to Z$使得分解$f=jg$成立，所以$f$是一个闭浸入。
...
\begin{pro}
设$\alpha:X'\to X$和$\beta:Y'\to Y$都是$S$-概形态射，如果它们都是（开、闭）浸入，则$\alpha\times_S\beta$也是。
\end{pro}
...
\begin{para}[对角线]
	设$X$是一个$S$-概形，则态射$(\id_X,\id_X):X\to X\times_S X$被称为对角线态射，记作$\Delta_{X|S}$，如果没有误解，则简单记作$\Delta_X$或$\Delta$.
\end{para}
...
\begin{pro}
	设$\{U_\alpha\}$是$Y$的一个开覆盖，则态射$f:X\to Y$分离当且仅当每个限制态射$f^{-1}(U_\alpha)\to U_\alpha$都分离。
\end{pro}

所以，分离性是一个局部条件。特别地，如果我们选取的是仿射开覆盖，则对分离性我们有如下刻画：

\begin{pro}
	设$Y$是一个仿射概形，而$X$是一个概形，$\{U_\alpha\}$是$X$的一个仿射开覆盖，则态射$f:X\to Y$分离当且仅当，对任意一组指标$(\alpha,\beta)$，$U_{\alpha\beta}:=U_\alpha\cap U_\beta$是仿射开集且环$\oo_X(U_{\alpha\beta})$可以由$\oo_X(U_{\alpha})|_{U_{\alpha\beta}}$和$\oo_X(U_{\beta})|_{U_{\alpha\beta}}$生成。
\end{pro}
