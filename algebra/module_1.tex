...
\begin{pro}
设$\mathcal{C}$是一个Abel范畴，则函子$\mathcal{C}(X,-):\mathcal{C}\to \mathsf{Ab}$以及$\mathcal{C}(-,X):\mathcal{C}^\circ \to \mathsf{Ab}$都是左正合函子。
\end{pro}

\begin{proof}
	设$0\to\cdot \xrightarrow{f}\cdot \xrightarrow{g}\cdot$是一个正合列，我们要证明
	\[
	0\to \cdot \xrightarrow{f_*}\cdot \xrightarrow{g_*}\cdot
	\]
	也是一个正合列。首先，我们说明$\ker f_*=0$. 由于是在$\mathsf{Ab}$中，所以考虑$f_*(g)=fg=0$，由于$f$是单态，所以$g=0$，因此$\ker f_*=f_*^{-1}(0)=0$. 然后，由于$g_*f_*=(gf)_*$，所以$\im f_*\subset \ker g_*$. 最后只要检验$\ker g_*\subset \im f_*$. 任取$h$使得$g_*(h)=gh=0$，由$g$的泛性质，存在分解$h=\ker(g)k=\im(f)k=fk=f_*(k)$，其中$\im(f)=f$来自于$f$是一个单态，所以$\ker (g_*)\subset \im (f_*)$.

	完全类似地，可以证明$\mathcal{C}(-,X):\mathcal{C}^\circ \to \mathsf{Ab}$是左正合函子。
\end{proof}
...
设$\mathcal{C}$是一个Abel范畴，而$X\in \mathcal{C}$是一个对象。考虑集合$\mathcal{C}(-,X)$，在其中可以定义关系$\sim$如下：设$f:Y\to X$和$g:Z\to Y$，如果存在一个$W\in \mathcal{C}$以及两个满态$u:W\to Y$以及$v:W\to Z$使得$fu=gv$，则记$f\sim g$. 以交换图记，就是说，$f\sim g$当且仅当存在满态$u$和$v$使得如下交换图成立，
\[
\xymatrix{
...
\begin{proof}[Proof of Lemma \thelem]
...
\begin{para}
从Lemma \ref{zhuitu}，我们得到了等价类$\mathcal{C}(-,X)/\sim$，其中的元素称为对象$X$的“元素”，记作$x$，而$x\in \mathcal{C}(-,X)/\sim$记作$x\in_m X$. 一般来说，我们会将等价类的元素心照不宣地看成一个指向$X$的箭头，即将$x$看成一个态射，或者反过来，将一个态射看成其等价类。比方说，$0$在$\mathcal{C}(-,X)/\sim$是看成$0$的等价类，$-x$是看成$-x$的等价类。值得注意的一点是，$0$的等价类中的元素都是$0$，即，如果$x\sim 0$，则$x=0$. 实际上，$x\sim 0$告诉我们存在满态$u$, $v$使得$xu=0v=0$，再由$u$是一个满态，所以$x=0$. 因此，$x\sim 0$等价于$x=0$.

设$f:X\to Y$是一个态射，$x\in_m X$，记$f(x)$是$fx$在$\mathcal{C}(-,Y)$中对应的等价类。不难看到这是良定的。于是，$f:X\to Y$给出了一个映射$\mathcal{C}(-,X)/\sim \to \mathcal{C}(-,Y)/\sim $，我们依然记作$f$. 特别地，$f(0)=0$，$f(x)=0$等价于$fx=0$.
\end{para}
...
\item $f$是满态，当且仅当任取$y\in_m Y$，都存在$x\in_m X$使得$y=f(x)$.
\item $f=0$当且仅当任取$x\in_m X$都有$f(x)=0$.
\item $\cdot \xrightarrow{f} \cdot \xrightarrow{g}\cdot$是正和的，当且仅当$gf=0$且如果$g(y)=0$，则存在$x$使得$y=f(x)$.
\item 如果$f(x)=f(x')$，则存在一个$x''\in X$使得$f(x'')=0$. 且对任意满足$g(x)=0$的态射$g$，我们都有$g(x'')=-g(x')$. 对任意满足$h(x')=0$的态射，我们都有$h(x'')=h(x)$.
\end{compactenum}
\end{pro}

第五点看着奇怪，实际上$x''$是模范畴中$x-x'$的类比。追图法的存在让我们可以像处理模范畴一样来处理Abel范畴中的交换图。
