...
\begin{thm}[Hamilton-Cayley定理]\label{Hamilton-Cayley}
...
作为Hamilton-Cayley定理的应用，我们有如下断言：
...
\para 设$M$是一个模，其子集$X$被称为线性无关的，如果任取其中有限个元素$x_{i_1},\dots,x_{i_k}\in X$，以及对任意的$r_{i_1},\dots,r_{i_k}\in R$，使得如果$r_{i_1}x_{i_1}+\cdots+r_{i_k}x_{i_k}=0$可以推出$r_{i_1}=\cdots=r_{i_k}=0$. 如果有一组线性无关的元素可以生成$M$，则$M$同构于那族元素生成的自由模，此时$M$称为被$X$自由生成的，这组元素被称为自由生成元，或者叫做一组基。\endpara
...
\begin{proof}
	...
	\]
	两边除以$a_{n+1}$得出$x_{n+1}$可以由$\{x_1,\dots,x_{n}\}$生成，于是$M$也可以由$\{x_1,\dots,x_{n}\}$生成，由归纳假设，$M$是一个自由模。
\end{proof}
...
$\mathfrak{m}$是一个清楚的有限生成$\mathcal{F}_p$-模，生成元为$\{x_1,\dots,x_n\}$. 现在设一族元素$\{f_1,\dots,f_n\}$，使得$\{df_1,\dots,df_n\}$生成了作为$\mathcal{F}_p/\mathfrak{m}=\mathbb{R}$-模的余切空间$\mathfrak{m}/\mathfrak{m}^2$.

应用Nakayama引理，我们可以知道$\{f_1,\dots,f_n\}$生成了$\mathfrak{m}$. 所以此时，$\{f_1,\dots,f_n\}$表现得就像局部坐标函数一样。这是在说什么呢？为此，将$\{df_1,\dots,df_n\}$按照$\{dx_1,\dots,dx_n\}$展开，有
\[
	df_i=\sum_{j=1}^n\partial_jf_i dx_j,
\]
其中展开系数构成的矩阵$(\partial_jf_i)$就是所谓的Jacobian. $\{df_1,\dots,df_n\}$生成$\mathfrak{m}/\mathfrak{m}^2$，此时当且仅当$(\partial_jf_i)$可逆。因此，Nakayama引理告诉我们，如果$(\partial_jf_i)$可逆，则$\{f_1,\dots,f_n\}$生成了$\mathfrak{m}$. 于是，对每一个$x_i$，我们有
\[
	x_i=\sum_{j=1}^n a_{ij}f_j.
\]
于是，我们就得到了$\{f_1,\dots,f_n\}$的“反函数”。这并不是真正的反函数，是因为系数$a_{ij}\in \mathcal{F}_p$还没写成$\{f_1,\dots,f_n\}$的函数形式。
...
% \para 设$M$是一个$R$-模，$S$是$R$的一个乘性子集，记$\varphi:M\to S^{-1}M$是局部化自然的同态，满足$\varphi(m)=m/1$. 设$N\subset M$是一个子模，则$S^{-1}N$是$S^{-1}M$的一个子模。由于子模的原像是子模，所以如果$L$是$S^{-1}M$的子模，则$\varphi^{-1}(L)$是$M$的子模。

\begin{lem}
	如果$N$是$M$的一个子模，则$N\subset \varphi^{-1}(S^{-1}N)$. 
\end{lem}

% 这个关系连同上面的一些关系非常类似Galois联络，只不过这里都不是逆序的而是保持顺序的。
...
设$M$是域$k$上的有限生成模，它是自由模，不妨直接写作$k^n$，一组自由生成元写作$\{1_1,\dots,1_n\}$，那么对小于$n$的正整数$m$，$\{1_1,\dots,1_m\}$自由生成了它的一个子模$k^m$. 于是我们有链
\[
...
\begin{thm}\label{finite_length_module}
...
这里稍微谈一下几何。我们可以考虑$R/\mathfrak{a}$的准素分解，$\mathfrak{a}$
可以写成一族准素理想的交，这种分解并不唯一，但是如果一旦分解是不可简化的，就是不能
再删减哪怕一个准素理想，那么每个准素理想所准的那些素理想互不相同，且这些素理想的
集合是唯一的，且这些素理想是包含$\mathfrak{a}$的极小素理想，进而
$(R/\mathfrak{a})_{\mathfrak p}$是Artin环，换而言之，是零维的。翻译成概形语言，
Noether仿射概形$\spec R$的一个闭子概形$Z$存在准素分解，
那些极小素理想对应的子概形叫做$Z$的不可约分支。
对一个不可约分支$\overline{\{x\}}$，$(\mathcal O_Z)_x$是零维的。
...
\begin{pro}[Artin-Rees引理]\label{artin-rees}
令$R$是一个Noether环，$\mathfrak a$是一个理想，$M$是一个有限生成
