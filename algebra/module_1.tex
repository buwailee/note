...
\]
被称为正合的。如果只满足$f_{i+1}f=0$，则称列是一个链复形，或者简称一条链。
\end{para}

显然，正合列是一个链。如果$\im f_i\approx \ker f_{i+1}$，则$\coker \ker f_{i+1} \approx \coker \im f_i$，换而言之，存在同构$t$使得$\coker \ker f_{i+1}=t\coker \im f_i$. 因此
\begin{align*}
