...
\end{proof}
 ...
 \begin{pro}
    设$M$是一个有限生成$R$-模且$\ann_R(M)=\langle p_1^{n_1}\cdots p_m^{n_m}\rangle \neq 0$，从而局部化映射$M\to M_{\langle p_i\rangle}$的核给出了$\langle p_i\rangle$-子模$M_i$，且$\bigcap_{i}M_i=\{0\}$.
\end{pro}
...
\begin{lem}
    设$M$是一个$R$-模且$\ann_R(M)=\langle p_1^{n_1}\cdots p_m^{n_m}\rangle \neq 0$，则存在一个$m_i\in M(p_i)$使得$\ann(m_i)=\langle p_i^{n_i}\rangle$，
\end{lem}
...
\begin{pro}\label{pro:6.3.6}
    设$R$-模$M$满足$\ann_R(M)$非零，则存在$m\in M$使得$\ann(m)=\ann(M)$.
\end{pro}
...
\begin{pro}
    设$M$是一个有限生成$R$-模且$\ann_R(M)=\langle p_1^{n_1}\cdots p_m^{n_m}\rangle \neq 0$，则
    \[
        M=M(p_1)\oplus \cdots  \oplus M(p_m).
    \]
\end{pro}

\begin{proof}
    取$m\in M$但$m\not\in M(p_1)\oplus \cdots  \oplus M(p_m)$. 由于$\ann_RM\subset \ann_R m=\langle q\rangle$，所以，可以设
    \[
        q=p_1^{l_1}\cdots p_m^{l_m},
    \]
    其中$l_i\leq n_i$.
\end{proof}
