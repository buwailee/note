...
\]
被称为正合的。在正合列中，平凡群一般写作$1$，零环和零模则写作$0$，从平凡群出发或者到平凡群的同态记作$1$，零环和零模的则记作$0$，通常略去他们的符号，因为这些同态的我们是清楚的。
...
我们抽象出这个性质：如果一个$G$的子群$H$满足对任意的$g\in G$都成立$gH=Hg$，则$H$被称为$G$的正规子群。
...
\proof
	...
	现在假设$f$是满同态，则$\bar{f}$是满同态。我们剩下的只要证明$\bar{f}$是个单射，或者$\ker(\bar{f})=\{\bar{e}\}$即可。设$\bar{f}(\bar{a})=f(a)=e$，于是$a\in \ker(f)=\bar{e}$，因此$\bar{a}\cap \bar{e}=\varnothing$，于是我们也就得到$\bar{a}=\bar{e}$，所以$\ker(\bar{f})=\{\bar{e}\}$. 
...
\para 从一个给定的范畴$\mathcal{C}$，可以构造出一个新的范畴$\mathcal{C}^\circ$如下：对象不变，而$\Hom_{\mathcal{C}^\circ}(X,Y)$被定义为$\Hom_{\mathcal{C}}(Y,X)$，复合不变。他被称为给定范畴的对偶范畴。
...
\para 在有些作者那里，我们上面定义的函子被称为协变函子，而函子$F:\mathcal{C}^\circ \to \mathcal{D}$被称为反变函子。后者明确写出来即$F(\varphi\psi)=F(\psi)F(\varphi)$，其中$\psi$和$\varphi$都是$\mathcal{C}$中的函子。
