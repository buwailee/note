...
\begin{pro}
准加性范畴中，始对象等价于终对象，所以也就都等价于零对象。
\end{pro}

\begin{proof}
设$A$是始对象或者是终对象，则$\mathcal{C}(A,A)$是单点集，因此$\id_A=0$. 反过来，假设$\id_A=0$，考虑$f\in \mathcal{C}(A,X)$，由于$f=f\id_A=f0=0$. 所以$A$是始对象。考虑$f\in \mathcal{C}(X,A)$，由于$f=\id_A f=0f=0$. 所以$A$是终对象。
\end{proof}
...
所以，经常我们会直接将$\ker(f)$看成一个态射，谈及对象的时候，只要取其定义域即可。$\coker(f)$同理。
...
\begin{compactenum}[~~~(1)]
\item 若$f$和$g$指向同一个对象$X$，若此时存在一个同构$\varphi$使得交换图成立
\[
\begin{xy}
	\xymatrix{
	\cdot\ar[d]_{\varphi}\ar[r]^-{f} &X\\
	\cdot \ar[ru]_{g}
	}
\end{xy}
\]
...
\item  若$f$和$g$都从同一个对象出发，若此时存在一个同构$\varphi$使得交换图成立
\[
\begin{xy}
	\xymatrix{
	X \ar[r]^-{f}\ar[rd]_{g} &\cdot \ar[d]^{\varphi}\\
	&\cdot 
	}
\end{xy}
\]
...
如果$\ker(f)_1$和$\ker(f)_2$都是一个态射$f$的核，则$\ker(f)_1\approx\ker(f)_2$. 

\end{para}

\begin{para}
在准加性范畴$\mathcal{C}$中，如果任意核都存在，则任意等值子都存在。实际上，$f$, $g$的等值子就是$\ker(f-g)$. 同理，如果任意余核存在，则任意余等值子存在。
\end{para}
...
\begin{para}
再一个抽象的例子，考察复合态射的核。设$f:X\to Y$以及$g:Y\to Z$是两个态射，那么按照在模范畴中的直觉，应当有一个单同态$h:\ker(f)\hookrightarrow \ker(gf)$. 放到范畴里，这也是正确的，只不过应该把单同态改成单态。存在性是由$\ker(gf)$的泛性质保证的，并且成立分解$\ker(f)=\ker(gf)h$，如果$hk_1=hk_2$，则$\ker(f)k_1=\ker(f)k_2$，因为$\ker(f)$是单态，所以$k_1=k_2$. 于是$h$是单态。
\end{para}

\begin{para}
类比熟知的$\coker f$的定义$N/\im f$，可以定义$\im f=\ker\left(\coker f\right)$. 
\end{para}

为使得这个定义是良定的，我们需要证明选取不同的$\coker f$，我们将得到同构的$\im f$. 为此，如果选取不同的$(\coker f)_1$和$(\coker f)_2$，有$(\coker f)_1\approx (\coker f)_2$，存在同构$\psi$使得$(\coker f)_2=\psi(\coker f)_1$.

由上一段，可知存在单态$h:\ker((\coker f)_1)\hookrightarrow \ker((\coker f)_2)$，同理$(\coker f)_1=\psi^{-1}(\coker f)_2$给出了单态$k:\ker((\coker f)_2)\hookrightarrow \ker((\coker f)_1)$，直接的验证将给出$h$和$k$是互逆的一对态射。这就是我们需要的同构。

由于$(\coker f) f=0$，所以由$\ker$的泛性质，我们得到了唯一的态射$M\to \im f$，这个态射类似于$\ker f$和$\coker f$，记作$\im f$.
...
\begin{lem}
设$X=\bigoplus_{a=1}^nX_a$，则任意$f:X\to X$可以由所有的$f_{ab}=p_afi_b:X_b\to X_a$确定。并且，如果有$g:X\to X$，则$(gf)_{ab}=\sum_{c}g_{ac}f_{cb}$.
\end{lem}
...
实际上，
\[
	\sum_{a,b}i_af_{ab}p_b=\sum_{a,b}i_ap_afi_bp_b=\left(\sum_ai_ap_a\right) f\left(\sum_bi_bp_b\right)=\id_{X}f\id_{X}=f.
\]
以及
\[
	(gf)_{ab}=p_agfi_b=p_ag\id_X fi_b=\sum_c p_ag i_cp_c fi_b=\sum_{c}g_{ac}f_{cb}.
\]
\end{proof}

第一点告诉我们，如果存在一族映射$f_{ab}:X_b\to X_a$，可以构造$f=\sum_{a,b}i_af_{ab}p_b:\bigoplus_{a=1}^nX_a\to \bigoplus_{a=1}^nX_a$使得$f_{ab}=p_afi_b$成立。特别地，如果只存在那些“对角”的态射$f_a:X_a\to X_a$，我们可以构造$f_1\oplus \cdots \oplus f_n:\bigoplus_{a=1}^nX_a\to \bigoplus_{a=1}^nX_a$使得
\[
	p_a(f_1\oplus \cdots \oplus f_n)i_a=f_a,
\]
其中
\[
	f_1\oplus \cdots \oplus f_n=\sum_{a}i_af_{a}p_a.
\]
...
\begin{proof}
由于$f\eq(f,g)=g\eq(f,g)$，如果$\eq(f,g)$是满态（右可消），如果$f=g$. 此时不难检验$\id_X$是$f$, $f:X\to Y$的一个等值子，由等值子的泛性质，$\eq(f,f)\approx \id_X$，即存在同构$\varphi$使得$\id_X=\eq(f,f)\varphi$. 最后，考虑$\eq(f,f)\varphi\eq(f,f)=\eq(f,f)$，由等值子的泛性质，分解的唯一性保证了$\varphi\eq(f,f)=\id_{\eq(f,f)}$. 因此$\eq(f,f)$是一个同构。
\end{proof}
