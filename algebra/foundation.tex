...
唯一性的部分命题直接来自于上面的引理。从上一个引理，我们知道存在这样的单态$t$. 现在，由于$\coim(f)=th$，左侧是一个满态，所以$t$是一个满态。一个单态如果是满态，则必然是一个同构。
\end{proof}
...
\begin{proof}
首先，我们假设$f$是一个单态，而$g$是一个满态。令$\varphi=\coker (f) \ker (g)$，我们验证$a=\ker \varphi$和$b=\coker \varphi$，即验证它满足泛性质。
...
\end{xy}
\]
由于$ta=0$等价与$ta'\coim(f)=0$等价于$ta'=0$，所以$\coker(a)=\coker(a')$. 同理（对偶地），我们有$b'$使得$b=\im(g)b'$且$\coim(g)=b' \coker(f)$，所以$\ker(b')=\ker(b)$.
