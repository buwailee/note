...
如果$R$中的乘法是可交换的，则称$R$是一个交换环。有些作者在环的定义中会去掉乘法单位元，称我们这里定义的环为含幺环。

和群不同的是，环乘法不一定是可逆的（即对$a$存在一个$b$使得$ab=1$），比如$0$，对于任意的$a\in R$都有$a0=0\neq 1$，所以$0$一定不是可逆的。如果一个环除了零之外的元素全部可逆，则称他为一个除环。交换除环被称为域(field)。
...
\para 在有些作者那里，我们上面定义的函子被称为协变函子，而函子$F:\mathcal{C}^\circ \to \mathcal{D}$被称为反变函子。后者明确写出来即$F(\varphi\psi)=F(\psi)F(\varphi)$，其中$\psi$和$\varphi$都是$\mathcal{C}$中的态射。
