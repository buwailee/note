...
\para[本征值与本征矢量] associated primes的经典应用是本征值问题。考虑一个非零$k$-矢量空间$V$，以及一个线性映射$A:V\to V$. 那么，我们可以通过$x\cdot v=Av$定义$V$的一个$k[x]$-模结构。

考虑最简单的情况，$k$是一个代数闭域，比方说$\mathbb C$. 此时，$\mathbb C[x]$中的素理想都形如$\langle x-a\rangle$. 此时，如果$\langle x-a\rangle=\ann(v)$是$V$的一个associated prime，则这就意味着$(A-a)v=0$，或等价地$Av=av$. 这样的一个$a$，我们称之为线性映射$A$的一个本征值，而$v$是对应本征值$a$的一个本征矢量。因为$k[x]$总是Noether环，所以上面的命题告诉我们associated prime一定存在，继而如果$k$是代数闭域，则我们还可以得到一个经典的结论：此时本征值以及其本征矢量一定存在。这个结论以往是在有限维的情况应用代数基本定理得到的，而我们这里的推理是可以直接应用到任意的矢量空间上的。

当然，对于非代数闭域，associated primes可能就不是一次式生成的（尽管依然是主理想），所以就可能不存在本征值。\endpara
...
利用这个命题，在本征值问题中（对线性变换$A$和矢量空间$V$），我们可以算出$A$有哪些（但可能不是全部）本征值。因为$k[x]$是PID，如果$\ann(V)$是一个非零理想，则其由一个多项式生成（被称为极小多项式），而这个多项式可以写成一些不可约多项式的乘积（可能重复），这些不可约多项式就生成了包含$\ann(V)$的极小素理想。如果是在代数闭域中，这些素理想都是一次式生成的，它们分别对应着$A$的本征值。
...
\begin{pro}
	设$M$是一个$R$-模，而$\pp\in \supp(M)$是一个包含$\ann(M)$的极小素理想，设$\alpha:M\to M_\pp$是局部化同态，则$\ker\alpha$是一个$\pp$-准素子模。
\end{pro}
...
\begin{thm}
	令$R$是Noether环，而$M$是有限生成$R$-模，$N$是$M$的一个真子模。如果$N$可以写成并$N=\bigcap_{i=1}^n M_i$，其中$M_i$是$M$的$\mathfrak p_i$-准素子模，记$P$为所有的$\mathfrak p_i$的集合，则
	\begin{compactenum}
