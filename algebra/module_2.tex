...
\begin{lem}
设$M$是一个$R$-模，如果$\ann(m)$是素理想，则对$M$的子模$Rm$的任意非零子模$N$有$\ann(N)=\ann(m)$.
\end{lem}
...
\para 设$N$是$M$的一个子模，如果$\Ass(M/N)$只有一个元素，则称呼$N$是$M$的一个准素子模，如果$\Ass(M/N)=\{\mathfrak{p}\}$，则称呼$N$是$\mathfrak{p}$-准素的。

如果设$N_1$和$N_2$都是$\mathfrak{p}$-准素的，则$N_1\cap N_2$也是. 事实上，考虑单同态$M/(N_1\cap N_2)\to M/N_1\oplus M/N_2$，因此$\Ass(M/(N_1\cap N_2))\subset \Ass(M/N_1)\cup \Ass(M/N_2)=\{\mathfrak{p}\}\cup \{\mathfrak{p}\}=\{\mathfrak{p}\}$. 但是由于$\Ass(M/(N_1\cap N_2))$非空，所以$\Ass(M/(N_1\cap N_2))=\{\mathfrak{p}\}$.
