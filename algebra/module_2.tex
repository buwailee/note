...
定义$\Ann(M)$为所有$\ann(m)$中的那些素理想构成的集合。当然可能所有的$\ann(m)$都不是素理想，但是如果$R$是一个Noether环，则下面的引理保证了$\Ann(M)$非空。
...
\proof
	由于单同态并不会改变$\Ann$，所以可以将$M'$看成$M$的子模，所以第一个包含显然。对第二个包含，任取$\mathfrak{a}\in \Ann(M)-\Ann(M')$，我们要证明$\mathfrak{a}\in \Ann(M')$. 设$\mathfrak{a}=\ann(m)$，其中$m\in M$，由$m$生成的子模$Rm$同构于$R/\mathfrak{a}$. 考虑$Rm$的任意非零子模$N$，对应着$R/\mathfrak{a}$的任意理想，因为$\mathfrak{a}$是素理想，所以$R/\mathfrak{a}$的任意非零理想都是整环，对应着$\ann(N)=\mathfrak{a}$. 考虑$N=Rm\cap M'$，因为$\mathfrak{a}\notin \Ann(M')$，$\mathfrak{a}N\neq 0$，所以只可能$Rm\cap M'=0$. 在这种情况下，$Rm$同构于他在$M''$中的像，所以$\mathfrak{a}\in \Ann(M'')$.
\qed

考虑$M=M'\oplus M''$的情况，由于$M'$和$M''$都有到$M$的单同态，所以$\Ann(M')\subset \Ann(M)$以及$\Ann(M'')\subset \Ann(M)$，继而他给出了$\Ann(M)=\Ann(M')\cup \Ann(M'')$.

\para 设$N$是$M$的一个子模，$\Ann(M/N)$是一个单点集，则称呼$N$是$M$的一个准素子模，如果$\Ann(M/N)=\{\mathfrak{p}\}$，则称呼$N$是$\mathfrak{p}$-准素的。

\pro 设$N_1$和$N_2$都是$\mathfrak{p}$-准素的，则$N_1\cap N_2$也是.

经过有限次归纳可以得到：有限个$\mathfrak{p}$-准素子模的交依然是一个$\mathfrak{p}$-准素子模。

\proof
	因为我们有单同态$M/(N_1\cap N_2)\to M/N_1\oplus M/N_2$，因此$\Ann(M/(N_1\cap N_2))\subset \Ann(M/N_1)\cup \Ann(M/N_2)=\{\mathfrak{p}\}\cup \{\mathfrak{p}\}=\{\mathfrak{p}\}$. 但是由于$\Ann(M/(N_1\cap N_2))$非空，所以$\Ann(M/(N_1\cap N_2))=\{\mathfrak{p}\}$.
\qed

准素子模的重要性来自于下面这个定理，他被称为准素分解。

\theo 设$R$是一个Noether环，而$M$是一个有限生成$R$-模，设$M'$是$M$的一个子模且$M'\neq M$，那么他是有限个准素理想的交。
