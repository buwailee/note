...
\usepackage{noteheader, indentfirst}
...
% 更紧要的是，式(\theequation)告诉我们，可以从一个1-形式$\omega$构造一个$2$-形式$\dd \omega$，通过所谓的外微分算符$\dd$.

% \para 当然我们可以把上面这些活做得更加形式一点，记$\Omega^2(M)$是流形$M$上（吃掉两个矢量场的）反对称函数的集合，线性算子$\dd:\Omega^1(M)\to \Omega^2(M)$通过式$\dd \omega (X,Y)=X(\omega(Y))-Y(\omega(X))-\omega([X,Y])$对任意矢量场$X$, $Y$都成立来定义，这样的$\dd$被称为外微分算子。

% 很久以前就定义过，光滑函数的外微分算子$\dd:\Omega^0(M)\to \Omega^1(M)$，现在又定义了光滑切矢量场$\dd:\Omega^1(M)\to \Omega^2(M)$.
...
特别地，如果$\omega'=\dd x'$和$\eta'=\dd y'$，且$x'$和$y'$可以看做$x$和$y$的函数，而
\[
...
% \para 再换个角度，在一点处$\mm_p^2/\mm_p^3$也有一个自然的矢量空间结构，他同构于$T_p^*M\otimes T_p^*M$的对称化，即设$\omega\otimes \eta-\eta\otimes \omega$生成了$I$，$\mm_p^2/\mm_p^3\cong T_p^*M\otimes T_p^*M/I$.
