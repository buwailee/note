...
\lem 单位分解的弱版本：设$(U,\varphi)$是$M$上的一个坐标卡，$V$是$M$上的开集，他的闭包$\bar{V}$紧，如果$\bar{V}\subset U$，则在流形$M$上存在光滑函数$h:M\to [0,1]$使得$h$在$V$内为$1$，在$U$外为$0$.
...
\para 先假设$\dd$在$U$上是存在的。若$\omega$是$V\osub U$上的$k$-形式，对于任意一点$p\in V$，找$p$的一个有紧闭包的足够小的可以容纳坐标的邻域$W$以及单位分解$h$，此时$h\omega$在$U$上有了定义（在$V$外为零，在$W$内等同于$\omega|_W$），于是我们可以诱导出$W$上的一个外微分算符$\dd_W$如下$\dd_W(\omega|_W)=\dd(h\omega)|_W$.

\para 设$V\osub U$，如果$\omega|_V=0$，则$(\dd \omega)|_V=0$.

实际上，任取一点$p$，找$p$的一个有紧闭包的足够小的可以容纳坐标的邻域$W_p$，所有的$W_p$定义了$V$的一个开覆盖。我们只要证明对其中的一个$W_p$，$(\dd \omega)|_{W_p}=0$即可。

同上利用单位分解找一个光滑函数$h:U\to [0,1]$，则$h\omega$在$U$上恒为零，所以
\[
	0=\dd (h\omega)=\dd h \wedge \omega +h\dd \omega
\]
限制在$W_p$上，就有$(\dd \omega)|_{W_p}=0$. 

利用这个结论，如果$W$在$U$中允许单位分解（这里并不指广义的单位分解的存在性，而是上面陈述的弱单位分解命题的成立），由于$\dd(h\omega)|_W-\dd \omega|_{W}=\dd(h\omega-\omega)|_{W}$，由于$(h\omega-\omega)|_W=0$，则$\dd(h\omega-\omega)|_{W}=0$，或者$\dd(h\omega)|_W=\dd \omega|_{W}$.

\para 于是，如果$W$在$U$中允许单位分解，连同$\dd_W$的定义，我们有$\dd_W(\omega|_W)=\dd \omega|_{W}$.或者，利用$k$-形式和$(k+1)$-形式的限制映射$\rho^k_{UW}$和$\rho^{k+1}_{UW}$，写作
\[
	\dd_W\circ \rho^k_{UW}=\rho^{k+1}_{UW}\circ \dd.
\]

\para 容易证明，以$p$的在$U$中允许单位分解的开集$W$赋予包含而成的归纳系，
...
\[
	\xymatrix{
	&&\ar[dl]^{\rho^k_{Vp}}\Gamma(\Omega^k,V)\ar[dd]^{\rho^k_{VW}}\ar@/_/[lld]_{\varphi^{k+1}_{Vp}\circ \dd_V} \\
	\Omega_p^{k+1}&\ar@{-->}[l]_(0.4){\dd_p}\Omega_p^{k}&\\
	&&\ar[ul]_{\rho^k_{Wp}}\Gamma(\Omega^k,W)\ar@/^/[llu]^{\varphi^{k+1}_{Wp}\circ \dd_W}
	}
\]

\pro 这样的映射$\dd$存在且唯一。

有了上面这些表述，我们只要在局部证明其唯一存在即可。
