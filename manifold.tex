...
\para 设$E$, $B$, $F$是三个光滑流形，$\pi:E\to B$是一个光滑映射，若在每一点$p\in B$，都存在一个邻域$U$使得$\pi^{-1}(U)\cong U \times F$，则称$(E, B, \pi, F)$是一个以$B$为底，以$F$为纤维的纤维丛。

一般称呼$B\times F$为平凡丛。

\para 设$(E, B, \pi, F)$是一个纤维丛，设$U\subset B$是一个开集，则$U$上的光滑截面定义为一个光滑映射$s:B\to E$满足$\pi\circ s=\id_U$.

一个纤维丛不一定有整体截面，但是一定有局部截面，因为纤维丛在局部都是平凡丛，而平反丛一定有截面，比如常值截面$s(x)=a\in F$.
...
\para 设$U\osub M$，$U$上的映射$\omega:p\mapsto \omega(p)\in T_p^*M$被称为$U$上的余切矢量场。因为在$U$的每一个局部$V$（至少一个坐标卡内），余切矢量场$\omega$都可以写作$\omega=a_i\dd x^i$，其中$a_i$是$V$上的实值函数，而$\dd x^i$在不同的点分属不同的余切空间。如果$\{a_i\}$在点$p$是光滑函数，则称$\omega$在$p$处光滑。如果$\omega$在$U$处处光滑，则称$\omega$是$U$上的一个光滑余切矢量场（或者叫做一个1-形式场）。
...
\para 外代数的复习在附录，对于矢量空间$V$的$k$-次外代数记做$\Omega^k(V)$。在流形上的一点$p$处，记$\Omega_p^k=\Omega^k(T_pM)$，则$\Omega_p^1=T_p^*M$.

\para 设$U\osub M$，$U$上的映射$\omega:p\mapsto \omega(p)\in \Omega_p^k$被称为$U$上的一个$k$-形式。称他是光滑的，如果他吃下任意$k$个光滑矢量场会得到一个光滑函数。所有$U$上的光滑$k$-形式的集合记做$\Omega^k(U)$.显然，$U$上的光滑函数可以看成一个光滑0-形式，一个光滑余切矢量场是一个光滑1-形式。如果$\omega$是一个光滑1-形式，则$D(\omega)$是一个光滑2-形式。下面我们所称的形式都是光滑的，我们将省略光滑二字。
...
\lem 单位分解的弱版本：设$(U,\varphi)$是$M$上的一个坐标卡，$V$是$M$上的开集，如果$\bar{V}\subset U$且$\bar{V}$紧，则在流形$M$上存在光滑函数$h:M\to [0,1]$使得$h$在$V$内为$1$，在$U$外为$0$.
...
\item 任取光滑函数$f$，$\dd^2 f=\dd (\dd f)=0$.
\item 作为微分算符的Leibniz法则：设$\omega$是$r$-形式，则
\[
...
\para 如果这样的映射$\dd$存在，则他是一个局部算符，即，如果$\omega$在$U$上为零，则$\dd \omega$也在$U$上为零。实际上，任取一点$p$，找$p$的一个有紧闭包的足够小的可以容纳坐标的邻域$V$，利用单位分解找一个光滑函数$h:M\to [0,1]$，则$h\omega$在$M$上恒为零，所以
\[
	0=\dd (h\omega)=\dd h \wedge \omega +h\dd \omega
\]
限制在$V$上，就有$\dd \omega|_V=0$，由于$p\in U$任取，所以$\dd \omega|_U=0$.因此，$\dd$在$p\in M$上可以诱导出一个$\dd_p$算符，使得$\dd_p:\Omega^k_p\to \Omega^{k+1}_p$.

有了这个性质，即使我们形式$\omega$只定义在一个比较小的开集上，我们也可以通过$p\mapsto \dd_p\omega$来定义出他的外微分。
