...
这样定义的$f^*$在一点诱导出的$f^*_p$可以验证和$1$-形式间已经存在的$f^*_p$用张量积诱导出来的映射是相同的，但是我们这里不采取这样每一点拼起来的方式。
...
\]
其中$(-1)^r$的出现来自楔积的交换。这个Leibniz法则我们已经在计算$D(f\dd g)$和$\dd(fg)$的时候遇到过了。
...
\]
不难检验外微分的性质都可以得到满足。这样我们就在局部证明了外微分的存在性。\qed
