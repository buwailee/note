...
% % \para 大概是无聊的一个定义：有一个非空集合$G$和其上的二元运算$*$，或者记作$(G,*)$，称为一个群，如果满足：
...
% 有些人说，群就是对称性，这并不是一个很好的说法。群在定义上不过是代数结构，群和对称性的联系来自于，保持某些结构的双射们（可逆的变换们）往往构成一个群。

% 实际上，在具体语境中对称性的定义会有不同，这来自于我们想要怎么去表述对称性。粗略来说，对称性大概就是一种在某些/类变换下不变（或者更广义地，按照某种希望的方式变换）的某种性质。所以我们有如下说法：

% 具有群$G$对称性的某物：\emph{指某物的某结构在变换群$G$的作用下不变（或者更广义地，按照某种希望的方式变换）。}


% \para 作为第一个例子，我们考虑$\mathrm{O}(n)$和$\mathrm{U}(n)$的定义。

% 设$\rr^n$上面具有一个内积$(v,w)=\sum_{i=1}^n v_i w_i$，而$\cc^n$上有一个内积$\langle v,w\rangle=\sum_{i=1}^n v^*_i w_i$，其中$v_i^*$是$v_i$的复共轭。从几何上面来看，内积给出了一个矢量的长度，以及两个实矢量之间的夹角。
...
% 类似地，考虑一个复矩阵$A:\cc^n \to \cc^n$，如果对任意的$v$和$w$都使得$\langle Av,Aw\rangle =\langle v,w\rangle $成立，则称$A$是一个幺正矩阵。同上可以类似验证所有的幺正矩阵构成一个群，这个群被记作$\mathrm{U}(n)$，称为酉群。
...
% \para 一个集合$G$，上面有一个运算$*$满足结合率$a*(b*c)=(a*b)*c$，且存在唯一一个元素$e$使得$a*e=e*a=a$对任意的$a\in G$都成立，同时，任取$g\in G$都存在一个元素$h\in G$使得$g*h=h*g=e$. 下面写群运算的时候直接无视掉他的符号，即$g*h$写作$gh$.
...
% \para 举个例子，三维矢量空间$\rr^n$，上面装备了一个内积$(v,w)=\sum_i v_iw_i$，这样的空间被称为$n$维Euclid空间。有了内积，我们可以通过内积定义出一个矢量的长度，以及两个矢量的夹角。

% 设矩阵$A$使得$(Av,Aw)$不变，那么这个变换使得内积不变。几何上来看，这个变换保持矢量的长度以及两个矢量的夹角。这样的矩阵被称为正交矩阵。不难检验，所有正交矩阵按矩阵乘法构成一个群，称为正交群，记作$\mathrm{O}(n)$. 所以，我们说，$n$维Euclid空间具有$\mathrm{O}(n)$对称性，他表现为$\mathrm{O}(n)$中的变换使得内积不变。
