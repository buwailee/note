...
\para 作为第一个例子，我们考虑一个矢量空间$V$，所有可逆线性变换（或者说矩阵）$A:V\to V$按照复合（或者说矩阵乘法）构成了一个群，他被记作$\operatorname{GL}(V)$，被称为一般线性群。当$V=\rr^n$的时候，一般线性群记作$\operatorname{GL}(n)$，当$V=\cc^n$的时候，一般线性群记作$\operatorname{GL}(n,\cc)$.

在$\operatorname{GL}(V)$中，那些行列式为$1$的矩阵构成了一个子群$operatornamem{SL}(V)$，因为行列式为1的矩阵的逆以及乘积的行列式都还是1. 从线性代数的几何直观，行列式为1的矩阵保持体积。
...
\para 现在来看$\operatorname{SL}(2,\cc)$. 任何一个复可逆$2\times 2$矩阵都可以唯一分解（极分解）为$
...
\para[Lie群] 若一个群$G$同时具有相容的群结构和微分流形结构，则群$G$是一个Lie群。所谓相容的，就是说，群运算是可微的，所谓的微分流形，就是广义的曲面。比如刚刚说的，$\operatorname{SL}(2,\cc)$在拓扑上等价于$\rr^4\times \mathbb{S}^3$，这是一个七维的曲面。
\endpara

粗略来说，Lie群就是那些在上面可以进行微积分的群\footnote{严格一点的定义，参见附录。}，$\operatorname{GL}(n)$和$\operatorname{GL}(n,\cc)$都是Lie群。对于矩阵群来说，上面的求导定义为
\[
...
\proof 我们来证明逆否命题，如果$\lag$有一个非平凡交换理想$\mathfrak{a}$，使得$[\mathfrak{a},\lag]\subset \mathfrak{a}$，则若证明了$(x,y)_K=0$对任意的$x\in \mathfrak{a}$和$y\in \lag$都成立，其自然告诉我们Killing形式是退化的。
