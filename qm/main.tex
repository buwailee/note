...
\newtheorem{para}{}[chapter]
	...
	\newcommand{\paracount}[1]{\refstepcounter{para}\textbf{#1 (\thechapter.\thepara)}\hspace{0.4em}}
	\renewcommand{\para}{\refstepcounter{para}\textbf{(\thechapter.\thepara)}\hspace{0.5em}}
	\newcommand{\theo}{\paracount{Theorem}}
	\newcommand{\pro}{\paracount{Proposition}}
	\newcommand{\lem}{\paracount{Lemma}}
	\newcommand{\cor}{\paracount{Corollary}}
	\renewcommand{\proof}{\textit{Proof}:\hspace{0.5em}}
...
\para 如果我们的内积空间$\mathcal{H}$在这个范数下是完备的（即Cauchy列收敛），则称$\mathcal{H}$是一个Hilbert空间。由于Cauchy列收敛，所以Hilbert空间的非空闭凸子集$E$\footnote{所谓凸集就是说，如果$x$和$y$都在该集合内，则$(x+y)/2$也在该集合内。}必然有着唯一的范数最小的元素。为了证明这点，考虑$E$中所有元素范数的下界$d$，然后找一个序列$\{x_i\}$使得$|x_i|\to d$，由于$(x_m+x_n)/2\in E$，故$|(x_m+x_n)/2|\geq d$，考虑恒等式
\[
...
\no{1} 对于单粒子，存在一个可分Hilbert空间$\mathcal{H}$，其中矢量被称为态矢量。物理上的态（可以理解为单粒子的所有物理信息）被态矢量所描述。并且，如果$\Psi$和$\Phi$是两个态矢量，满足$\Phi = a \Psi$，其中$a$是一个非零复常数，则他们蕴含的物理信息是相同的。
